...
class TuneGeneratorApp {
    ...
    loadDefaultValues() {
        ...
        document.getElementById('displacement').value = 350;
        document.getElementById('cylinders').value = 8;
        document.getElementById('compression').value = 9.0;
        document.getElementById('fuelPressure').value = 43.5;
        document.getElementById('revLimit').value = 6500;
    }
    ...
    loadKnownCombo() {
        ...
        if (!comboKey || comboKey === 'custom') {
            document.getElementById('knownComboInfo').style.display = 'none';
            return;
        }
        ...
        document.getElementById('displacement').value = combo.engine.displacement;
        document.getElementById('cylinders').value = combo.engine.cylinders;
        document.getElementById('compression').value = combo.engine.compression;
        document.getElementById('engineFamily').value = combo.engine.family;
        
        // Populate fuel specs
        document.getElementById('injectorModel').value = combo.fuel.injectorModel;
        document.getElementById('fuelPressure').value = combo.fuel.fuelPressure;
        document.getElementById('fuelType').value = combo.fuel.fuelType;
        
        // Populate ignition specs
        document.getElementById('ignitionCoil').value = combo.ignition.coilType;
        document.getElementById('revLimit').value = combo.ignition.revLimit;
        
        // Populate camshaft specs
        document.getElementById('camProfile').value = combo.camshaft.profile;
    ...
    loadInjectorSpecs() {
        ...
        if (injectorSelect.value === 'custom') {
            customSpecs.style.display = 'block';
            specsDisplay.innerHTML = '';
            return;
        }
        
        if (injectorSelect.value === 'unknown') {
            customSpecs.style.display = 'none';
            specsDisplay.innerHTML = '<p><strong>Unknown injectors:</strong> Will use estimated values based on engine displacement.</p>';
            return;
        }
    ...
    loadCoilSpecs() {
        ...
        if (!coilSelect.value || coilSelect.value === 'unknown') {
            specsDisplay.innerHTML = '<p><strong>Unknown coil:</strong> Will use conservative ignition settings.</p>';
            return;
        }
        ...
        if (revLimitInput.value > coilData.maxRPM) {
    ...
    loadWidebandSpecs() {
        ...
        if (!widebandSelect.value || widebandSelect.value === 'unknown') {
            specsDisplay.innerHTML = '<p><strong>Unknown wideband:</strong> Will use default O2 sensor calibration.</p>';
            return;
        }
    ...
    // Toggle custom cam specs
    toggleCustomCam() {
        const camProfile = document.getElementById('camProfile').value;
        const customSpecs = document.getElementById('customCamSpecs');
        
        customSpecs.style.display = camProfile === 'custom' ? 'block' : 'none';
    }
    
    // Toggle unknown checkboxes
    ...
    validateEngineSpecs() {
        ...
        if (!document.getElementById('compressionUnknown').checked) {
        ...
        summaryContainer.innerHTML = `
            ...
            <div class="summary-grid">
                <div class="summary-item">
                    <strong>Engine:</strong> ${this.formData.displacement}${this.formData.displacementUnit} ${this.formData.cylinders}-cyl<br>
                ...
                <div class="summary-item">
                    ...
                    <strong>Intended Use:</strong> ${this.formData.intendedUse.replace('_', ' ')}
                </div>
            </div>
        `;
    }
    ...
    generateTune() {
        ...
        try {
            ...
            let documentation = '';
            if (document.getElementById('includeDocumentation').checked) {
...
function selectSetupType(type) {
    window.app.selectSetupType(type);
}
...
function generateTune() {
    window.app.generateTune();
}

function loadKnownCombo() {
    window.app.loadKnownCombo();
}

function toggleCustomCam() {
    window.app.toggleCustomCam();
}

function toggleUnknown(fieldName) {
    window.app.toggleUnknown(fieldName);
}

function loadInjectorSpecs() {
    window.app.loadInjectorSpecs();
}

function loadCoilSpecs() {
    window.app.loadCoilSpecs();
}

function loadWidebandSpecs() {
    window.app.loadWidebandSpecs();
}

function loadPumpSpecs() {
    window.app.loadPumpSpecs();
}

function updateTimingRecommendation() {
    window.app.updateTimingRecommendation();
}

function updateAFRRecommendation() {
    window.app.updateAFRRecommendation();
}
